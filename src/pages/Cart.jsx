...
const Cart = () => {
  ...
  <tbody>
   {ctx.cartArr.map((ele,index)=>{
    ...
    <td>{ele.title}</td>
    <td>{ele.price}</td>
    <td><button onClick={()=>handleIncrement(ele)} className='btn btn-outline-info'>+</button>{ele.quantity} <button className='btn btn-outline-info' onClick={()=>handleDecrement(ele)} >-</button></td>
